...
- **Engagement classifier**: `gemini-2.5-pro` with `responseSchema` returning `{ state, confidence, rationale, cues }`. `thinkingBudget: 1024`, `maxOutputTokens: 2048`. Lives in `src/layers/classifier.ts`; exposed at `POST /api/engagement/classify`.
