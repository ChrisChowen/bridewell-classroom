...
export function Fleur({ size = 16, title = "" }: { size?: number; title?: string }) {
  return (
    ...
    >
      ...
      {/* Tiny heraldic-red dot pulled from the crest — the only red in
          most of the UI. At ~1.2px this reads as a calligraphic mark,
          not a warning. */}
      <circle cx="12" cy="13" r="1.2" fill="var(--color-bridewell-red)" />
    </svg>
  );
}
