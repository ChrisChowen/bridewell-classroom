...
- **Tom Reeves** — `wheel_spinning` — pressed scaffolds and gave terse echoes ("They're absorbed?").
- **Sophie Renton** — a **safeguarding flag at high severity** with the verbatim
  pupil excerpt ("my mum was drunk again and I had to look after my little
  brother…"). A teacher would see this in seconds.
- **Marcus Holt** — `flowing` — answered confidently and elaborated unprompted.
- **Bertie Lawson** — terse one-word answers ("Leaves.") surface as `disengaged`.
...
a single typed entry point in `src/lib/ai/llm.ts`, with a named key for **each
distinct job — none conflated** (`tutor`, `scaffold`, `classifierFlash`,
`classifier`, `reasonEvaluator`, `profileUpdater`, `lessonPlanner`, `appraiser`,
`sessionClose`). The current build maps those keys to a fast tier (tutor +
scaffolding + first-pass classifier) and a reasoning tier (classifier
tiebreaker, Reason evaluator, lesson planner, close-of-lesson consolidation,
appraisal). A production deployment points each key — independently, by config
alone — at whichever managed and guardrailed model the schools have a contract
with; **zero changes** in `src/layers/` or the routes. For Bridewell that is the existing
...
This build is engineered to be **plug-and-played into Unified Projects'
existing Bridewell AI infrastructure** with documented, swappable seams — not
just demoed. Two documents are the contract:

- **`HANDOVER.md`** — the integration contract: what Unified inherits, what they
  must provide (model endpoints, identity, data residency), and the exact files
  that constitute each seam. Target: a competent engineer repoints the model +
  auth + data seams to their own infrastructure in under a day.
- **`PILOT_READINESS.md`** — an honest green/amber/red ledger across every gate
  (engineering, GDPR, safeguarding, consent, DPA). Anything needing a human is
  amber/red with the human action named — never silently green.

**Three provider seams**, each behind one interface so the backend can be
substituted by config alone:

- **Model** — `src/lib/ai/llm.ts` + `src/lib/ai/models.ts` (`LLM_PROVIDER`); every
  call-type has its own key (see above). Proven swappable by a unit test.
- **Auth** — `src/lib/auth/` (`AUTH_PROVIDER`); `verifyRequest(req)` is the only
  thing routes call, so Entra/SAML/OIDC federation slots in behind it.
- **Data** — `src/lib/data/` (`DATA_PROVIDER`); entities read/write through a
  `DataStore` interface, not raw Firestore.

**Compliance, drafted for human sign-off (never claimed done):**

- **GDPR** — subject-access export (Art. 15, `src/lib/pupil-data.ts` +
  `/api/pupils/{id}/export`) and full erasure (Art. 17, `/api/pupils/{id}/delete`,
  emulator-proven total + isolated). Pseudonymised export filenames; no PII in
  logs. Drafts: `docs/DPIA-draft.md`, `docs/data-retention-policy.md`,
  `docs/DPA-draft.md`, `docs/consent-and-privacy-copy-draft.md`.
- **Safeguarding** — medium/high disclosures write a permanent audit-trail doc
  with reviewer/timestamp/note; routing policy drafted in
  `docs/safeguarding-routing-policy.md` for DSL sign-off.
- **Hardened auth** — Firestore + RTDB rules scope a teacher to only their own
  classes/pupils; the projector reads a names-stripped aggregate keyed by a
...
- **responder.ts** — pure function. Picks the next tutor turn based on the branch.
  ...
  follow-up the evaluator drafted. Pattern flag → a neutral, continuative line to
...
| Persona | Intended signal | Classifier returned |
|---|---|---|
...
| Sophie Renton (safeguarding-disclosure) | medium/high safeguarding | **safeguarding HIGH** raised + audited ✓ |
| Jacob Pritchard (off-task / withdrawn) | off_task / disengaged | **disengaged 100%** ✓ |
| Marcus Holt (flowing) | flowing | **flowing 90%** ✓ |
| Bertie Lawson (disengaged) | disengaged | **disengaged 90%** — terse-correct distinguished from substantively-engaged after a classifier prompt revision ✓ |
...
The classroom-display mode — **"The Field"** — is the front-of-room projector
view. The teacher's own dashboard stays on their device; the projector shows a
**collective** surface for the room: a constellation of star points (one per
pupil, coloured by engagement state) drifting over a calm navy field, a colour
key in non-judgemental language, and a slow moments ticker ("the class is
finding limiting factors tricky"). **No individual pupil names** are ever
surfaced here — the projector reads only the names-stripped aggregate node.
Open it from the class detail header — *Whiteboard*:
...
- **Next.js 15.5** App Router · **React 19** · **TypeScript strict** · **Tailwind 4**
  (pinned to 15.x — Next 16 + Turbopack breaks `firebase-admin` in the deployed
  Cloud Function; documented in `CLAUDE.md`)
...
src/
  app/
    ...
    api/
      ...
      pupils/me             — pupil session reads its class (+ adaptive + SEND)
      pupils/[id]/export    — GDPR Art. 15 subject-access export (teacher-scoped)
      pupils/[id]/delete    — GDPR Art. 17 erasure (teacher-scoped, audited)
      pupils/[id]/send      — read/write a pupil's SEND adaptation profile
      pupils/[id]/profile   — teacher override of adaptive challenge level
      engagement/classify   — classify a transcript (synchronous)
      engagement/run        — full live classifier loop (snapshot + RTDB + aggregate)
      ...
      session/consolidate   — class-end pupil consolidation (reasoning tier)
      lessons/generate      — AI-led lesson plan
      lessons/intent-suggestions — starting-point phrasings for the brief step
      lessons/appraise      — post-class appraisal of the plan
      lessons/save-to-library — validated write to the shared library
      lessons/library       — list library entries (rated)
      tts                   — British-English ElevenLabs speech (opt-in)
      admin/allowlist(+bulk)— teacher allowlist admin (admin teachers only)
      admin/teachers        — roster + enable/disable + password reset
      admin/cost            — 14-day LLM cost aggregate (admin only)
  components/
  ...
  lib/
    ai/
      llm.ts                — typed LLM client + seam (grounding, structured JSON)
      models.ts             — provider-agnostic model-key map (one key per job)
      prompts.ts            — tutor system-prompt builder (+ text-only/injection guards)
      output-guards.ts      — runtime check the tutor never promises visuals
    ...
    auth/                   — auth SEAM: verifyRequest + swappable provider (AUTH_PROVIDER)
    data/                   — data SEAM: DataStore interface + Firestore adapter (DATA_PROVIDER)
    cost/                   — per-call LLM cost estimator + daily aggregate recorder
    pupil-data.ts           — GDPR gather (Art. 15) + erasure (Art. 17)
    send.ts                 — SEND profile → tutor adaptation block
    learner-profile.ts      — adaptive-difficulty drift engine (pure)
    learner-profile-store.ts— longitudinal profile consolidation
    rate-limit.ts           — durable per-route limiter (RTDB-backed)
    voice/                  — British-English TTS/STT selection
...
- **Plug-and-play seams** — model, auth, and data each behind one swappable
  interface; `HANDOVER.md` integration contract; model-swap proven by test
- **Compliance** — GDPR subject-access export + erasure (emulator-proven),
  hardened Firestore/RTDB rules, durable rate limiting, safeguarding audit
  trail; DPIA / DPA / retention / consent drafts for human sign-off; a live
  `PILOT_READINESS.md` ledger
- **Test + CI foundation** — 150+ Vitest unit tests + emulator rules/GDPR tests,
  gated by GitHub Actions on every push
- **Capability** — adaptive per-pupil difficulty, longitudinal learner profile
  (`/api/session/consolidate`), British-English voice I/O, SEND→prompt
  adaptation + pupil accessibility menu, cross-subject syllabi
- **Tutor safety** — hard text-only capability block + prompt-injection guards
  across every pupil-facing generation path (verified against the live model:
  the tutor never promises diagrams it can't draw, jailbreaks held)
- **A round of live bug-fixes** — incl. the pupil-join flow (a deployed
  read-after-write visibility lag was bouncing first-time joins back to the code
  screen; the join now carries its result forward so `/session` renders without
  an immediate read), accessibility (chat live region, modal focus traps),
  resilience (graceful LLM fallback, sign-in timeout), and UX polish

What landed during the v1 build week:
