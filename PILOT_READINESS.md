...
| **RTDB live-mirror read scoping** | 🔴 | **KNOWN EXPOSURE.** `database.rules.json` `liveSessions/$sessionId/.read: auth != null` lets ANY authed user (incl. a pupil in another class, or any anonymous projector client) read another class's full live mirror — pupil names, engagement states, **safeguarding severity**. Violates guardrails 3 + 7. Fix needs a redesign: split per-pupil node (teacher + that pupil only, via denormalised teacherId/membership) from a name-free aggregate node the projector reads. Risky to rush; next focused increment. |
