...
| Data seam behind one interface | 🟡 | **Seam built + proven swappable; ALL entity reads migrated.** `DataStore` interface + Firestore adapter + registry + `resolveDataStore()` (`src/lib/data/`), `DATA_PROVIDER`-selected, mirroring the model/auth seams (swap proven by `src/lib/data/index.test.ts`). **Every single-doc `classes`/`pupils`/`learnerProfiles` read across the API now goes through `getClass`/`getPupil`/`getLearnerProfile`** — `grep` confirms zero direct `collection("…").doc().get()` entity reads remain in `src/app/api`. **Single-doc writes now on the seam too:** `savePupil` + `saveLearnerProfile` (merge-aware) added to the interface/adapter; the join pupil-write, SEND-profile write, and learner-profile teacher-override write all route through them (swap proven by an added write test). _Remaining (the honest gap): the **atomic class+join-code batch** (class create) and the injected-db consolidate write stay direct by design (they need a transaction-aware seam primitive); multi-doc **queries** (where-clauses) + the RTDB live mirror are also still direct. So a `DATA_PROVIDER` swap now covers all entity reads + the simple single-doc writes, but not batches/queries/RTDB yet._ |
...
| GDPR: subject-access export (Art. 15) | 🟢 | `gatherPupilData` (`src/lib/pupil-data.ts`) + teacher-scoped `GET /api/pupils/{id}/export`. Emulator-tested (`npm run test:emulator`). **Teacher-facing download button live** in the pupil drill-down (`LivePupilPanel` → "Data (GDPR)" → downloads the JSON via the token-scoped endpoint). _Erasure (Art. 17) teacher button — confirm-echo modal — remains a deliberately-guarded follow-up._ |
| GDPR: full pupil-record deletion (Art. 17) | 🟢🔒 | `deletePupilData` + teacher-scoped `POST /api/pupils/{id}/delete` (requires confirmPupilId echo). Emulator-proven **total + isolated** (target → 0 docs incl. the names-stripped RTDB `aggregate` slot, other pupil untouched; `rtdbNodes: 3`). Ownership enforced server-side (`pupil-auth.ts`). _Teacher-facing confirm-modal button: small follow-up._ **🔒 Open policy question (DPO + DSL):** erasure currently hard-deletes `safeguardingEvents` too. Statutory safeguarding-retention duties may *require* those records to survive a pupil-erasure request (GDPR erasure vs safeguarding retention is a known tension). Whether to retain (and for how long, under what lawful basis) is a human decision — flagged here, not silently changed either way. |
...
| **RTDB live-mirror read scoping** | 🟡 | `database.rules.json` v2: `liveSessions/{id}/pupils` readable only by the owning teacher (via denormalised `meta/teacherId`); each pupil reads only their own entry + own interventions; `status` + name-free `aggregate` stay authed-readable. Closes the cross-class names/engagement/**safeguarding** leak. Emulator-tested (`npm run test:rtdb`, 11 assertions); `meta/teacherId` written at class-create + self-healed in engagement/run + backfilled for existing classes. **Projector now works no-login:** `engagement/run` populates the names-stripped `aggregate` node (state/confidence/step/recency + one concern boolean, keyed by a non-reversible UID hash); the `/classroom` whiteboard reads it after anonymous sign-in — no PII reaches the public screen. **v3 staged, not yet deployed (🔒 needs `firebase deploy --only database` on review):** pupil write to `liveSessions/{id}/pupils/{uid}` is tightened from full-node to the two optimistic numeric leaves (`liveMessageCount`, `lastMessageAt`) only — closing a hole where a pupil could overwrite their own engagement state or clear a safeguarding flag. The matching client change (leaf-only `bumpPupilLiveMessage`) is already live on the branch and works under both old and new rules; **production stays open on this until the rules are deployed.** RTDB rules are global (not per-channel), so they were intentionally NOT pushed from the autonomous branch. |
...
| Per-class/teacher cost tracking | 🟡 | **Per-call LLM usage + estimated cost** recorded best-effort: callLLM exposes a pure `setUsageRecorder` hook (model seam stays dependency-free); a server-only recorder (`src/lib/cost/`) is registered via a side-effect import in the nodejs LLM routes (NOT `instrumentation.ts` — that edge-compiles and can't carry firebase-admin), incrementing a daily `costTracking/{day}` aggregate by model key. Pure estimator unit-tested. Admin surface shows 14-day spend + calls + per-day table (`GET /api/admin/cost`, est. only). Fire-and-forget → zero added latency. _Per-class/teacher attribution (vs per-model-key) is the next step — needs class/teacher context threaded to the recorder._ |
| Multi-teacher within a school (roster import, co-teacher) | 🟡 | **Registered-teacher roster + account management** in the admin surface: `GET /api/admin/teachers` lists teachers with admin + **enabled/disabled** status (status from Firebase Auth, batched `getUsers`); `POST` **disables/re-enables** an account (`updateUser({disabled})`, with a self-lockout guard); per-teacher **password reset**. **Bulk roster import:** `POST /api/admin/allowlist/bulk` (admin-only) adds up to 500 staff emails / `*@domain` wildcards in one batch, returning an added/skipped/invalid summary; the admin page has a paste-a-list importer. Entry validation (`src/lib/admin/allowlist-entry.ts`) unit-tested. Multiple teachers per school already work (each owns N classes). _Remaining: co-teacher / substitute access to a *shared* class._ |
...
| Anonymised research export (brief item N) | 🔴 | **Not built.** The only export today is the per-pupil GDPR Art.15 JSON (raw PII by design — row 33). Brief item N wants an *anonymised* CSV/JSON of session logs / engagement trajectories / scaffolding + Reason + intervention events across a class, pseudonymised `P001…`. This is a net-new research-export feature with real design surface (pseudonym-map persistence, class- vs school-scope, and **CSV-injection escaping** — prefix `=,+,-,@` cells with `'`). Flagged as the most significant remaining scope gap; deferred because the pseudonymisation scheme is a research-design decision, not a pure engineering one. |
