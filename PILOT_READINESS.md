...
| Auth seam behind one interface | 🟡 | **Seam built + proven swappable.** `AuthProvider` interface + Firebase adapter + registry + `verifyRequest(req,{role})` (header) **and `verifyAuthToken(token,{role})` (raw/body token)** helpers (`src/lib/auth/`), mirroring the model seam — `AUTH_PROVIDER` selects the backend; Entra/OIDC is a config+adapter swap with zero route changes. Swap proven by `src/lib/auth/index.test.ts` (**11 tests**: both entry points, stub-provider flow, role gate, 401/403/500 mapping). Migrated: `pupil-auth.ts` (export/delete/profile/send) **+ the 5 pupil-session body-token routes** (reason/fire, reason/evaluate, engagement/run, conversation/append, session/consolidate) — none call `verifyIdToken` directly now. _Remaining: ~9 teacher-role routes (lessons/*, classes/*, interventions, admin/allowlist, auth/teacher)._ |
| Data seam behind one interface | 🟡 | **Seam built + proven swappable.** `DataStore` interface + Firestore adapter + registry + `resolveDataStore()` (`src/lib/data/`), `DATA_PROVIDER`-selected, mirroring the model/auth seams. Covers the cross-route entity reads (class, pupil, learner profile); swap proven by `src/lib/data/index.test.ts` (3 tests). `GET /api/pupils/me` migrated to use **both** seams (auth + data) — a zero-direct-Firebase reference route. _Remaining: fold the analytics-collection writes + RTDB live mirror behind the interface and migrate the other routes._ |
